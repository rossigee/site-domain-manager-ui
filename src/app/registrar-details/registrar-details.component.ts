...
export class RegistrarDetailsComponent implements OnInit, AfterViewInit {
  ...
  handleCSVFileInput(files: FileList) {
    ...
    this.registrarService.postCSVFile(this.id, files.item(0)).subscribe(
      data => {
        ...
        if(data.status == "ok") {
          this.toastService.notice("Upload successful. " + data.records_read + " records processed.");
        }
        this.refresh();
      },
      error => {
  ...
  refreshFromAPI() {
    ...
    this.registrarService.refreshFromAPI(this.id).subscribe(
      data => {
        ...
        if(data.status == "ok") {
          this.toastService.notice("Refresh successful. " + data.records_read + " records updated.");
        }
        this.refresh();
      },
      error => {
  ...
  refresh = function() {
    this.registrarService.getRegistrar(this.id)
      .subscribe(
        resp => {
          this.loading = false;
