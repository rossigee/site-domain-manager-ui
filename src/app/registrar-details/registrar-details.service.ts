...
export class RegistrarDetailsService {
  ...
  constructor(
    ...
    this.handleError = httpErrorHandler.createHandleError('RegistrarDetailsService');
  }
  ...
  getRegistrar(id: string): Observable<Registrar> {
    ...
    return this.http.get<Registrar>(this.registrarsUrl + "/" + id, options)
      .pipe(
        catchError(this.handleError<Registrar>('getRegistrar'))
      );
  }
  ...
  postCSVFile(id: string, fileToUpload: File): Observable<PostResponse> {
    ...
    return this.http.post<PostResponse>(endpoint, formData, options)
      .pipe(
        map(res => {
          console.log(res);
          if(res.status == "ok")
  ...
  refreshFromAPI(id: string): Observable<PostResponse> {
    ...
    return this.http.get<PostResponse>(endpoint, options)
      .pipe(
        map(res => {
          console.log(res);
          if(res.status == "ok")
